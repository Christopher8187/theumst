...
const variant = shallowRef<Variant>((['A','B','C'].includes(params.get('variant') || '') ? params.get('variant') : 'A') as Variant);
...
const journey = shallowRef<'up'|'down'|null>(params.get('arrival') !== '0' && !reducedMotion ? 'up' : null);
...
const motes = Array.from({length:20},(_,i)=>({left:((i*47+13)%100)+'%',top:((i*31+17)%100)+'%',animationDelay:(-i*.7)+'s'}));

function updateUrl() {
  const query = new URLSearchParams(location.search);
  query.set('prototype','wisdom'); query.set('revision','2'); query.set('variant',variant.value);
...
function journeyDone() {
  if(journey.value==='down') location.assign('https://theumst.com/');
  else journey.value=null;
}
function keydown(event:KeyboardEvent) {
  if(journey.value || (event.target as Element)?.closest('input,textarea,select,[contenteditable=true]')) return;
...
<template>
  <main class="wisdom-prototype" :class="['interface-'+variant,{'showing-realms':scene==='realms'}]">
    <div class="sanctuary-art" aria-hidden="true"><img :src="dawn" alt=""><div class="sanctuary-light"></div><i v-for="(style,i) in motes" :key="i" class="sanctuary-mote" :style="style"></i></div>
    <div class="sanctuary-interface" :inert="!!journey">
      ...
      <div v-if="scene==='tree'">
        <component v-if="shown.length" :is="variants[variant]" :books="shown" :selected="chosenBook" :added="hasAdded" @select="select" @add="add" @enter="scene='realms'"/>
      ...
      <footer class="scene-navigation"><button class="return-home" @click="journey='down'"><span>↙</span> Return home</button><span class="scene-coordinate" aria-hidden="true">✧</span><button class="replay-arrival" @click="journey='up'">↑ Replay ascent</button></footer>
    ...
    <ArrivalTransition v-if="journey" :key="journey" :destination="dawn" :celestial="false" :sanctuary="true" :direction="journey" @done="journeyDone"/>
  </main>
</template>
