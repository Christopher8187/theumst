...
<style scoped>
/* Preserve scale and height throughout travel; clamp the altar crop to the image. */
.journey-tree{overflow:hidden;--tree-frame-width:max(100vw,calc(100dvh * var(--tree-frame-ratio)));--tree-focus:.35;--panorama-width:calc(var(--tree-frame-width) * var(--tree-image-factor));--tree-start:calc((100vw - var(--tree-frame-width)) * var(--tree-focus));--altar-end:clamp(calc(100vw - var(--panorama-width)),calc(55vw - var(--panorama-width) * .875),0px)}
.journey-tree img{position:absolute;display:block;max-width:none;width:var(--panorama-width);height:calc(var(--tree-frame-width) / var(--tree-frame-ratio));left:var(--tree-start);top:calc((100dvh - var(--tree-frame-width) / var(--tree-frame-ratio)) * .5);object-fit:fill;transform:translate3d(calc((var(--altar-end) - var(--tree-start)) * var(--altar-pan)),0,0);will-change:transform}
