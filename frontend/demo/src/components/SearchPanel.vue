...
<template>
  <section class="search-panel" role="region" aria-labelledby="similar-panel-title" tabindex="-1" @keydown.esc.stop="$emit('close')">
    <header><div><p v-if="kind!=='dependencies'" class="demo-kicker">{{ t.embeddingSearch }}</p><h2 id="similar-panel-title">{{ kind==='dependencies'?t.dependencies:kind==='crystallize'?t.crystallization:t.cluster }}</h2></div><button ref="closeButton" type="button" :aria-label="t.close" @click="$emit('close')">×</button></header>
    <p class="search-method">{{ kind==='dependencies'?t.dependenciesMethod:kind==='crystallize'?t.crystalMethod:t.similarityMethod }}</p>
    ...
    <div v-else-if="status === 'results'" class="resonance-list" :aria-label="kind==='dependencies'?t.dependencies:t.similarResults">
      <button v-for="result in results" :key="`${result.grimoire_id}:${result.knowledge_id}`" type="button" @click="$emit('open', result)">
        ...
        <i v-if="kind!=='crystallize' && kind!=='dependencies'">{{ t.similarityStrength }} {{ Number.isFinite(result.similarity_score) ? result.similarity_score.toFixed(3) : '—' }}</i>
        <i v-else-if="kind==='crystallize' && result.is_default_in_crystal">{{ t.preferredInstance }}</i>
      </button>
    </div>
