...
<template>
  <div class="notebook-math">
    <div v-for="(row, position) in rows" :key="position" class="notebook-math-row" :class="`notebook-${row.type}-row`"><span v-html="row.html"></span><slot v-if="position === rows.length - 1 && row.type === 'text'"></slot></div>
    <div v-if="$slots.default && rows.at(-1)?.type !== 'text'" class="notebook-math-row notebook-text-row"><slot></slot></div>
  </div>
</template>
...
.notebook-display-row>span{width:100%;min-width:0}
