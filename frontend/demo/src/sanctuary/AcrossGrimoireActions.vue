...
defineProps<{count:number;collectionActive:boolean;notesActive?:boolean}>();
defineEmits<{collection:[];notes:[];action:[id:string]}>();
...
<template>
  <nav class="across-group" :aria-label="t.acrossGrimoires">
    ...
    <div class="across-options">
      ...
      <button class="global-notes-action" :aria-pressed="notesActive" :title="t.notesHelp" @click="$emit('notes')">
        <span class="action-mark" aria-hidden="true"><i>寫</i></span><strong>{{t.notes}}</strong>
      </button>
...
@media(max-width:700px){
  ...
  .scope-label{display:none}.across-options{display:grid;grid-template-columns:repeat(6,minmax(0,1fr));padding:5px;gap:3px;border-radius:4px 17px 4px 4px}
