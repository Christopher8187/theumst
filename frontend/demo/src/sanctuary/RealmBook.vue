...
const emit = defineEmits<{ back: []; unavailable: []; realm: [value: RealmId | null]; completion: [bookId:string,count:number] }>();
...
const supportedRealm = (id?: RealmId) => id && ['text','questions','notes'].includes(id) ? id : null;
const destination = shallowRef<RealmId | null>(supportedRealm(props.initialRealm));
const notice = shallowRef('');
const inStudy = computed(() => ['text','questions','notes'].includes(destination.value || ''));
...
function activate(id: RealmId) {
  ...
  if (!supportedRealm(id)) {
    notice.value = `${t.value[id]} · ${t.value.comingSoon}`;
    clearTimeout(timer);
    timer = setTimeout(() => notice.value = '', 3200);
    return;
  }
  lastRealm.value = id;
...
<template>
  <section class="altar-interface" :class="{'realm-entering':enteringRealm,'in-realm':destination,'returning-from-realm':returning,'book-reforming':reforming}" :style="{'--realm-accent':accent}" :inert="!ready" :aria-hidden="!ready" @keydown.esc.stop="escape">
    ...
    <TextRealm v-if="inStudy" :book="book" :initial-notes="destination==='notes'" :initial-mode="destination==='questions'?'questions':'text'" @mode="studyModeChanged" @back="returnToBook" @unavailable="emit('unavailable')" @completion="(id,count)=>emit('completion',id,count)"/>
    <p v-if="notice" class="realm-notice" role="status">{{notice}}</p>
  </section>
</template>

<style scoped>
.realm-notice{position:fixed;bottom:24px;left:50%;transform:translateX(-50%);padding:12px 20px;background:#173448f5;border:1px solid #bededb70;z-index:50;font-size:13px}
...
@media(max-width:700px) and (max-height:660px) and (min-height:501px){.book-projection{top:calc(16px + env(safe-area-inset-top))}}
</style>
