...
const props = defineProps<{ book: SampleBook; initialMode?: 'text' | 'questions'; initialNotes?: boolean }>();
const emit = defineEmits<{ back: []; unavailable: []; completion: [bookId: string, count: number]; mode: [mode: 'text' | 'questions'] }>();
...
const state = useTextStudy(bookId, copy, (id: string, count: number) => emit('completion', id, count), toRef(props, 'initialMode'), () => emit('unavailable'), toRef(props, 'initialNotes'));
const { book, sampleBook, nodes, sections, selectedId, selectedNode, mode, sideMode,
  ...
  discoveryKind, originBookId, activeImage, canBack, canContinue, guard, actionBusy, toast, graphStatus, graphError, availability, loading, error } = state;
watch(mode, value => emit('mode', value));
const title = useTemplateRef<HTMLElement>('title');
function back() { state.run(() => {
  if (!showingAllNotes.value && originBookId.value && book.value.grimoire_id !== originBookId.value) return state.returnOrigin();
  else emit('back');
}); }
function trapAvailability(event: KeyboardEvent) {
  const buttons = [...(event.currentTarget as Element).querySelectorAll<HTMLButtonElement>('button')];
  if (buttons.length < 1) return;
  const at = buttons.indexOf(document.activeElement as HTMLButtonElement);
  event.preventDefault(); buttons[(at + (event.shiftKey ? -1 : 1) + buttons.length) % buttons.length].focus();
}
function escape(event: KeyboardEvent) {
  if (event.target instanceof Element && event.target.closest('.atlas-settings')) return;
  if (guard.pending.value) return;
  if (sideMode.value !== 'graph') { event.stopPropagation(); event.preventDefault(); state.run(state.closeSide); return; }
  if (showingAllNotes.value || sideMode.value === 'graph') { event.stopPropagation(); event.preventDefault(); back(); }
}
watch(availability, async value => { if(value){await nextTick();document.querySelector<HTMLButtonElement>('.availability-backdrop button')?.focus()} });
onMounted(async () => { await nextTick(); title.value?.focus({ preventScroll: true }); });
</script>

<template>
  <section class="text-realm notebook-study notebook-B notebook-selected" :class="{'all-notes-open':showingAllNotes}" @click.capture="state.captureExit" @keydown.esc.capture="escape" :aria-label="showingAllNotes?t.notes:mode==='questions'?t.questions:t.text">
    <RealmFolio class="text-folio" stacked :inert="actionBusy || !!guard.pending.value || !!availability">
    <nav v-if="showingAllNotes" class="text-topnav" :inert="actionBusy || !!guard.pending.value || !!availability">
      ...
      <StudyView v-if="!showingAllNotes" notebook :t="copy" :book="book" :nodes="nodes" :sections="sections"
        ...
        :notes="notes" :graph="graph" :graph-status="graphStatus" :graph-error="graphError" @retry-graph="state.retryGraph" @retry-discovery="state.run(state.action,discoveryKind)"
        ...
        <template #study-heading>
          <FolioNavigation :title="book.title" :back-label="originBookId && book.grimoire_id!==originBookId?t.returnOrigin:t.realmReturn" @back="back"/>
    ...
    <div v-if="availability" class="availability-backdrop" @keydown.tab="trapAvailability"><section role="dialog" aria-modal="true" :aria-label="t.sourceUnavailable"><h2>{{t.sourceUnavailable}}</h2><p>{{availability.base?t.originUnavailable:t.baseUnavailable}}</p><button @click="state.run(state.exitUnavailable)">{{t.confirmExit}}</button><button v-if="availability.base" @click="state.stayAvailable">{{t.stayBase}}</button></section></div>
    <div v-if="loading || error" class="text-notice" role="status">{{error || t.loadingLibrary}}</div>
    <NoteDecision :t="copy" :guard="guard"/>
  </section>
</template>

<style scoped>
.availability-backdrop{position:fixed;inset:0;z-index:1200;display:grid;place-items:center;background:#091c36a8;backdrop-filter:blur(12px)}.availability-backdrop section{width:min(480px,90vw);padding:28px;background:#183849f5;border:1px solid #bedade70;border-radius:3px 13px 3px 3px;color:#e1eaec}.availability-backdrop h2{font:italic 22px Georgia,serif}.availability-backdrop p{line-height:1.6}.availability-backdrop button{padding:10px 15px;margin:12px 10px 0 0;border:1px solid #a9cbd359;background:#aacdd610;color:#dbe9e6}
.text-folio :deep(.discovery-retry){align-self:flex-start;padding:10px 16px;color:#d7e9e7;background:#15364465;border:1px solid #a4cbd84d;border-radius:2px}
.text-realm{position:absolute;inset:22px 2.7% 3.5%;color:#e2ebee;font:13px/1.5 'Bahnschrift','Segoe UI',sans-serif;--text-ink:#e1eaec;--text-muted:#acc3ce;--text-cyan:#b4e4e2;--text-line:#b9d9dd33;animation:text-unfold .5s ease-out}
...
.text-folio{position:absolute;inset:64px 0 0}
...
.text-folio :deep(.math-content){font:16px/1.85 Georgia,'Times New Roman',serif;color:#edf0ec;overflow-wrap:anywhere}.text-folio :deep(.math-content p){margin:0 0 1em}.text-folio :deep(.math-content .katex){font-size:1.04em}.text-folio :deep(.katex-display){overflow-x:auto;overflow-y:hidden;padding:5px 0;margin:14px 0;text-align:left}.text-folio :deep(.katex-display>.katex){text-align:center}.text-folio :deep(.sound-corner){display:block;margin-left:auto;margin-top:8px}.text-folio :deep(.knowledge-images){display:flex;flex-wrap:wrap;gap:8px;margin-top:12px}.text-folio :deep(.knowledge-images button){padding:6px 10px;border:1px solid #b5d4df4a;border-radius:2px;color:#c0dce3;background:#a3d2d508;font-size:11px}
