...
<template>
  <section class="realm-destination" @keydown.esc.stop="emit('back')" :aria-label="t[realm]">
    ...
    <div class="destination-title"><h1 ref="title" tabindex="-1">{{t[realm]}}</h1></div>
    <div class="destination-later"><RealmDiagram :kind="realm"/><p>{{t[purposeKey(realm)]}}</p><small>{{t.comingSoon}}</small></div>
  </section>
</template>
