...
import { useGrimoireLibrary } from './useGrimoireLibrary';
...
const selectedId=shallowRef(params.get('book')||'');
...
const library=useGrimoireLibrary();
const { books:sampleBooks, added, loading, error, busy }=library;
...
const globalNotesOpen=shallowRef(false);
...
function updateUrl(){
  ...
  query.delete('prototype');query.delete('revision');query.delete('scenario');query.delete('notebook');query.delete('variant');query.delete('brief');query.delete('actions');query.set('place',place.value);query.set('view',scene.value);
...
function openGlobalNotes(){readerOpen.value=false;globalNotesOpen.value=true;updateTitle()}
async function closeGlobalNotes(){globalNotesOpen.value=false;updateTitle();await nextTick();document.querySelector<HTMLButtonElement>('.global-notes-action')?.focus({preventScroll:true})}
function openBrief(id:string){library.run(async()=>{await library.detail(id);select(id);readerOpen.value=true})}
function add(){if(chosenBook.value&&!hasAdded.value)library.run(()=>library.add(chosenBook.value!.id))}
...
function enter(id=chosenBook.value?.id){if(!id||altarMoving.value)return;library.run(async()=>{await library.detail(id);await library.add(id);select(id);readerOpen.value=false;activeRealm.value=null;departureScene.value=onlyMine.value?'collection':'tree';scene.value='realms';updateUrl();altar.travel(1)})}
...
const homepageUrl=location.port==='5175'?`${location.protocol}//${location.hostname}:5173/`:'/';
async function leaveUnavailable(){departureScene.value='collection';goTree();await library.load()}
function journeyDone(){const descended=journey.value==='down';place.value=descended?'home':'tree';journey.value=null;updateUrl();if(descended)location.assign(location.port==='5175'?`${location.protocol}//${location.hostname}:5173/`:'/')}
...
function keydown(event:KeyboardEvent){
  if(journey.value||altarMoving.value||scene.value==='realms'||readerOpen.value||globalNotesOpen.value||place.value==='home'||(event.target as Element)?.closest('input,textarea,select,[contenteditable=true],.across-group'))return;
...
function updateTitle(){document.title='Theumst · '+(globalNotesOpen.value?t.value.notes:readerOpen.value?t.value.brief:scene.value==='realms'?(activeRealm.value?t.value[activeRealm.value]:t.value.realmBook):t.value.tree)}
watch([lang,scene,activeRealm,readerOpen,globalNotesOpen],updateTitle);
watch(readerOpen,updateUrl);
onMounted(async()=>{await library.load();if(chosenBook.value)await library.run(()=>library.detail(chosenBook.value!.id));updateTitle();const icon=document.createElement('link');icon.id='wisdom-favicon';icon.rel='icon';icon.href=logo;document.head.append(icon);window.addEventListener('keydown',keydown);updateUrl()});
...
<template>
  <main class="wisdom-prototype interface-B actions-E chosen-folio" :class="{'brief-open':readerOpen,'showing-realms':scene==='realms','showing-collection':onlyMine,'at-home':place==='home','travelling':!!journey}" :data-altar-position="altarPosition.toFixed(3)">
    ...
    <div class="home-landing" :style="{opacity:homeExposure}" :inert="busy||!!journey||place!=='home'" :aria-hidden="place!=='home'||!!journey"><img class="landing-logo" :src="logo" alt="Theumst"><div class="home-threshold"><button @click="travel('up')">{{t.ascend}} <span>↑</span></button><a :href="homepageUrl">{{t.openHomepage}} ↗</a></div></div>
    <div class="sanctuary-interface" :style="{opacity:treeExposure*entranceExposure}" :inert="busy||!!journey||place!=='tree'||scene==='realms'||globalNotesOpen" :aria-hidden="place!=='tree'||!!journey||scene==='realms'||globalNotesOpen">
      ...
      <div v-if="!onlyMine"><OrbitVariant v-if="shown.length" :books="shown" :selected="chosenBook" :added="hasAdded" @select="select" @add="add" @enter="enter()" @preview="openBrief(chosenBook!.id)"/><section v-else class="empty-grimoires"><span>⌕</span><h2>{{search.trim()?t.noMatches:t.libraryUnavailable}}</h2><p v-if="search.trim()">{{t.trySearch}}</p><button v-if="search.trim()" @click="search=''">{{t.clearSearch}}</button></section></div>
      <GrimoireCollection v-else-if="onlyMine" :books="shown" :total="collectionTotal" :query="collectionSearch" @brief="openBrief" @enter="enter" @remove="id=>library.run(()=>remove(id))" @back="goTree" @clear="collectionSearch=''"/>
      ...
      <AcrossGrimoireActions :count="collectionTotal" :collection-active="onlyMine" :notes-active="globalNotesOpen" @collection="toggleCollection" @notes="openGlobalNotes" @action="notifySoon"/>

    </div>
    <RealmBook v-if="scene==='realms'&&chosenBook" :key="chosenBook.id" :book="chosenBook" :initial-realm="activeRealm||undefined" :reveal="bookExposure" :ready="!altarMoving&&altarPosition===1" @back="goTree" @unavailable="leaveUnavailable" @realm="changeRealm" @completion="updateCompletion"/>
    ...
    <GlobalNotes v-if="globalNotesOpen" @close="closeGlobalNotes"/>
    <div v-if="loading || error" class="wisdom-toast" role="status">{{error || t.loadingLibrary}}<button v-if="error" @click="library.load">{{t.retry || 'Retry'}}</button></div>
