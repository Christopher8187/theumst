...
import { createProjectionStore, fetchSimilarKnowledge, isUnavailable } from "./services/studyApi";
...
const projectionStatus = ref("idle");
...
async function loadProjectionForSelection(bookId, knowledgeId, options = {}) {
  const request = ++projectionRequest;
  const cached = projectionStore.current(bookId);
  ...
  try {
    const manifest = await projectionStore.load(bookId, { force: Boolean(options.force) });
    if (request !== projectionRequest) return;
    projection.value = manifest;
    projectionStatus.value = manifest.total_nodes ? "ready" : "empty";
    const withSelectedChunk = await projectionStore.ensureSelection(
      bookId,
      knowledgeId,
      manifest.projection_revision
    );
    if (request !== projectionRequest) return;
    projection.value = withSelectedChunk;
    projectionStatus.value = withSelectedChunk.total_nodes ? "ready" : "empty";
  } catch (reason) {
    if (request !== projectionRequest) return;
    projectionStatus.value = isUnavailable(reason) ? "unavailable" : "error";
    projectionError.value = reason?.message || t.value.graphError;
  }
}
...
async function selectNode(id, persist = true) {
  ...
  if (selectedBook.value?.grimoire_id) {
    void loadProjectionForSelection(selectedBook.value.grimoire_id, id);
...
function retryProjection() {
  const bookId = selectedBook.value?.grimoire_id;
  if (bookId == null || selectedId.value == null) return;
  void loadProjectionForSelection(bookId, selectedId.value, { force: true });
}

async function requestProjectionChunk(requestedChunk) {
  const bookId = selectedBook.value?.grimoire_id;
  const chunkIndex = Number(
    typeof requestedChunk === "object"
      ? requestedChunk?.chunkIndex ?? requestedChunk?.chunk_index
      : requestedChunk
  );
  const revision = projection.value?.projection_revision;
  if (bookId == null || revision == null || !Number.isInteger(chunkIndex) || chunkIndex < 0) return;
  try {
    const nextProjection = await projectionStore.ensureChunk(bookId, chunkIndex, revision);
    if (String(selectedBook.value?.grimoire_id) !== String(bookId)
      || String(projection.value?.projection_revision) !== String(revision)) return;
    projection.value = nextProjection;
    projectionStatus.value = nextProjection.total_nodes ? "ready" : "empty";
    projectionError.value = "";
  } catch (reason) {
    if (String(selectedBook.value?.grimoire_id) !== String(bookId)) return;
    projectionStatus.value = isUnavailable(reason) ? "unavailable" : "error";
    projectionError.value = reason?.message || t.value.graphError;
  }
}
...
<template>
  <div class="demo-shell">
    ...
    <main class="demo-main">
      ...
      <StudyView v-else-if="view === 'study'" :t="t" :book="selectedBook" :nodes="nodes" :selected-id="selectedId" :selected-node="selectedNode" :mode="mode" :side-mode="sideMode" :active-image="activeImage" :notes="notes" :similar-results="similarResults" :similar-status="similarStatus" :similar-error="similarError" :projection="projection" :projection-status="projectionStatus" :projection-error="projectionError" :origin-book-id="originBookId" @select="selectNode" @request-chunk="requestProjectionChunk" @retry="retryProjection" @soon="comingSoon" @action="action" @close-side="closeSide" @open-image="openImage" @save-note="saveNote" @notes-page="loadAllNotes().then(() => view = 'notes')" @open-result="openSimilarResult" @return-origin="returnOrigin" @move="moveExercise" @done="markDone" />
