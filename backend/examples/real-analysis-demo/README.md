...
The demo reads the complete book order and requests the Atlas graph view for the prototype's 42 stored dependencies. Reading rows and Hierarchy display those dependencies separately from gold reading-order arrows. The object manifest uses `source_metadata` for its order fields so ingestion preserves the intended sequence. Semantic Find neighbors requires real embeddings, which the original prototype does not provide.
