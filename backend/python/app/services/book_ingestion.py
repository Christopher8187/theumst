...
def _validate_manifest(manifest: dict[str, Any]) -> None:
    ...
    knowledge_graph = manifest.get("knowledge_graph")
    ...
    if knowledge_graph is not None:
        if int(manifest.get("schema_version") or 0) < 2:
            raise HTTPException(status_code=422, detail="knowledge_graph requires ingestion manifest schema_version 2")
        if not isinstance(knowledge_graph, dict):
            raise HTTPException(status_code=422, detail="knowledge_graph must be an object")
        if "archive_path" in knowledge_graph:
            _required_text(knowledge_graph.get("archive_path"), "knowledge_graph.archive_path")
            if knowledge_graph.get("format") not in (None, GRAPH_SIDECAR_FORMAT):
                raise HTTPException(status_code=422, detail="Unsupported knowledge_graph format")
            if knowledge_graph.get("compression") not in (None, "gzip"):
                raise HTTPException(status_code=422, detail="Unsupported knowledge_graph compression")
        elif "nodes" not in knowledge_graph or "relations" not in knowledge_graph:
            raise HTTPException(
                status_code=422,
                detail="knowledge_graph must be an inline sidecar or archive descriptor",
            )
...
def _graph_sidecar(bundle: ArchiveBundle) -> ValidatedGraphSidecar | None:
    descriptor = bundle.manifest.get("knowledge_graph")
    if descriptor is None:
        return None
    if "archive_path" not in descriptor:
        sidecar = descriptor
    else:
        archive_path = _required_text(descriptor.get("archive_path"), "knowledge_graph.archive_path")
        member = bundle.members.get(archive_path)
        if member is None:
            raise HTTPException(status_code=422, detail=f"Missing knowledge_graph file {archive_path!r}")
        try:
            with bundle.archive.open(member) as archived:
                ...
                with binary, io.TextIOWrapper(binary, encoding="utf-8") as text:
                    sidecar = _normalize_strings(json.load(text))
        except (UnicodeDecodeError, json.JSONDecodeError, OSError) as exc:
            raise HTTPException(status_code=422, detail="knowledge_graph sidecar is invalid JSON") from exc
    try:
        return validate_graph_sidecar(sidecar, bundle.manifest["objects"])
    except GraphSidecarValidationError as exc:
        raise HTTPException(
            status_code=422,
            detail={
...
async def ingest_book_archive(
    upload: UploadFile,
    *,
    actor_user_id: int | None = None,
) -> dict[str, Any]:
    ensure_policy_ready(user_id=actor_user_id, surface="ingestion")
    await upload.seek(0)
    bundle = _open_archive(upload.file)
    try:
        manifest = bundle.manifest
        _validate_manifest(manifest)
        graph = _graph_sidecar(bundle)
        graph_receipt = None
        graph_capability = "unchanged"
        with transaction() as (_, cur):
            source_key = _required_text(manifest["book"].get("source_key"), "book.source_key")
            cur.execute(
                "SELECT grimoire_id FROM grimoire WHERE source_key = %s",
                (source_key,),
            )
            existing_book = cur.fetchone()
            if existing_book:
                existing_id = int(existing_book["grimoire_id"])
            ...
            if graph is not None:
                graph_receipt = apply_graph_sidecar(
                    cur,
                    grimoire_id=grimoire_id,
                    book_source_key=str(manifest["book"]["source_key"]),
                    objects=objects,
                    graph=graph,
                    batch_size=DATABASE_BATCH_SIZE,
                )
                graph_capability = graph_receipt.capability
    ...
    return {
        ...
        "knowledge_graph": graph_receipt.as_dict() if graph_receipt is not None else {
