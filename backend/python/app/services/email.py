...
def ensure_email_configured(settings: Settings | None = None) -> Settings:
    ...
    if not settings.smtp_host or not settings.smtp_from_email:
        raise EmailConfigurationError(
            "Account email is not configured. Set SMTP_HOST and SMTP_FROM_EMAIL."
        )
    return settings
...
def build_email_verification_message(
    ...
    *,
    ...
    settings = ensure_email_configured(settings)
    message = EmailMessage()
    message["Subject"] = "Confirm your theumst email address"
    message["From"] = f"{settings.smtp_from_name} <{settings.smtp_from_email}>"
    message["To"] = recipient
    message.set_content(
        ...
        f"This link expires in {settings.email_verification_ttl_hours} hours and can only be used once.\n\n"
    ...
    message.add_alternative(
        ...
        <html lang="en">
          <body style="margin:0;background:#070a12;color:#e9edff;font-family:Arial,sans-serif">
            <div style="max-width:560px;margin:0 auto;padding:40px 24px">
              <p style="color:#8da2ff;font-size:12px;font-weight:700;letter-spacing:.12em;text-transform:uppercase">theumst account security</p>
              <h1 style="font-size:30px;line-height:1.15;margin:12px 0 18px">Confirm your email address</h1>
              <p style="color:#b9c2d8;line-height:1.7">Confirm this email address for your theumst account.</p>
              <p style="color:#b9c2d8;line-height:1.7">Confirmation only shows that you can access this inbox. It does not verify your identity or grant access to restricted material.</p>
              <p style="margin:28px 0">
                <a href="{verification_url}" style="display:inline-block;padding:14px 20px;border-radius:8px;background:#eef1ff;color:#0a1022;font-weight:700;text-decoration:none">Confirm email address</a>
              </p>
              <p style="color:#8d97ad;line-height:1.6;font-size:13px">This one-use link expires in {settings.email_verification_ttl_hours} hours. If you did not create this account, you can ignore this email.</p>
            </div>
          </body>
        </html>
        """,
...
def send_email_verification_email(
    ...
    *,
    ...
    settings = ensure_email_configured(settings)
    message = build_email_verification_message(recipient, verification_url, settings=settings)
    context = ssl.create_default_context()
