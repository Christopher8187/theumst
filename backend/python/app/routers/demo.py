...
def demo_access(request: Request, response: Response):
    ...
    # Only this successful bootstrap establishes/replaces the client token.
    # The middleware captured it before body generation and every other
    # protected response exposes the same value for comparison only.
    for name, value in getattr(request.state, "protected_access_headers", {}).items():
...
def _book_rows(cur, user_id: int, query: str = "") -> list[dict[str, Any]]:
    if not ensure_policy_ready(
        ...
        return []
    visibility = demo_corpus_visibility_sql("g.grimoire_id")
    cur.execute(
        f"""
        ...
        WHERE (%s = '' OR lower(lg.title) LIKE '%%' || lower(%s) || '%%')
          AND {visibility}
        ...
        ORDER BY COALESCE(ug.last_opened_at, 'epoch'::timestamptz) DESC,
                 lower(lg.title)
        """,
        (user_id, user_id, query, query, *corpus_visibility_params(user_id)),
    )
...
def _graph_cache_headers(etag: str, *, protected: bool = False) -> dict[str, str]:
    return {
        "ETag": etag,
        "Cache-Control": (
            "private, no-store" if protected
            else f"private, max-age={GRAPH_CACHE_SECONDS}"
        ),
...
def get_grimoire_graph(
    ...
    user = _demo_user(request)
    user_id = actor_user_id(user)
    ensure_policy_ready(user_id=user_id, surface="demo_graph")
    ...
    with transaction() as (_, cur):
        ...
        cur.execute(
            f"""
            SELECT EXISTS (
                SELECT 1 FROM research_corpus_policy rcp
                WHERE rcp.grimoire_id = g.grimoire_id AND rcp.is_protected
            ) AS is_protected
            FROM grimoire g
    ...
    headers = _graph_cache_headers(
        etag,
        protected=bool(book_access.get("is_protected")),
    )
...
def summon_grimoire(grimoire_id: int, request: Request):
    user = _demo_user(request)
    user_id = actor_user_id(user)
    ensure_policy_ready(user_id=user_id, surface="demo_book")
    with transaction() as (_, cur):
        visibility = demo_corpus_visibility_sql("g.grimoire_id")
        cur.execute(
            f"""
            SELECT 1 FROM grimoire g
            WHERE g.grimoire_id = %s
              AND {visibility}
            """,
            (grimoire_id, *corpus_visibility_params(user_id)),
...
def get_grimoire_knowledge(grimoire_id: int, request: Request):
    user = _demo_user(request)
    user_id = actor_user_id(user)
    ensure_policy_ready(user_id=user_id, surface="demo_knowledge")
    with transaction() as (_, cur):
        visibility = demo_corpus_visibility_sql("g.grimoire_id")
        cur.execute(
            f"""
...
def get_grimoire_knowledge_detail(grimoire_id: int, knowledge_id: int, request: Request):
    ...
    ensure_policy_ready(user_id=user_id, surface="demo_knowledge")
    with transaction() as (_, cur):
        visibility = demo_corpus_visibility_sql("s.grimoire_id")
        cur.execute(
            f"""
            SELECT k.knowledge_id, k.section_id, k.type, k.source_key,
                   k.source_metadata->'order' AS source_order,
        ...
        node["images"] = secure_image_rows(cur.fetchall())
...
def update_progress(
    ...
    user = _demo_user(request)
    user_id = actor_user_id(user)
    ensure_policy_ready(user_id=user_id, surface="demo_progress")
    with transaction() as (_, cur):
        visibility = demo_corpus_visibility_sql("s.grimoire_id")
        cur.execute(
            f"""
            ...
            WHERE k.knowledge_id = %s AND ug.user_id = %s
              AND {visibility}
            """,
            (knowledge_id, user["user_id"], *corpus_visibility_params(user_id)),
        )
        if not cur.fetchone():
            raise_hidden_knowledge_miss(
                cur,
...
def update_study_state(
    ...
    user = _demo_user(request)
    user_id = actor_user_id(user)
    ensure_policy_ready(user_id=user_id, surface="demo_state")
    with transaction() as (_, cur):
        visibility = demo_corpus_visibility_sql("s.grimoire_id")
        cur.execute(
            f"""
...
def list_notes(
    ...
    clauses.append(
        f"(n.grimoire_id IS NULL OR {demo_corpus_visibility_sql('n.grimoire_id')})"
    )
    values.extend(corpus_visibility_params(user_id))
    with transaction() as (_, cur):
        cur.execute(
            f"""
...
def create_note(payload: DemoNotePayload, request: Request):
    user = _demo_user(request)
    user_id = actor_user_id(user)
    ensure_policy_ready(user_id=user_id, surface="demo_note")
    with transaction() as (_, cur):
        ...
        elif payload.grimoire_id is not None:
            visibility = demo_corpus_visibility_sql("g.grimoire_id")
            cur.execute(
                f"""
                SELECT 1
                ...
                """,
                (
                    user["user_id"], payload.grimoire_id,
                    *corpus_visibility_params(user_id),
                ),
            )
            if not cur.fetchone():
...
def update_note(note_id: int, payload: DemoNotePayload, request: Request):
    ...
    ensure_policy_ready(user_id=user_id, surface="demo_note")
    with transaction() as (_, cur):
        note_visibility = demo_corpus_visibility_sql("n.grimoire_id")
        cur.execute(
            f"""
            SELECT 1 FROM demo_note n
            WHERE n.demo_note_id = %s AND n.user_id = %s
              AND (n.grimoire_id IS NULL OR {note_visibility})
            """,
            (note_id, user["user_id"], *corpus_visibility_params(user_id)),
        )
        if not cur.fetchone():
            raise_hidden_note_miss(
                cur,
        ...
        if payload.knowledge_id is not None:
            target_visibility = demo_corpus_visibility_sql("s.grimoire_id")
            cur.execute(
                f"""
                SELECT s.grimoire_id
                FROM knowledge k
            ...
            target_visibility = demo_corpus_visibility_sql("g.grimoire_id")
            cur.execute(
                f"""
                SELECT 1
                FROM grimoire g
                JOIN user_grimoire ug
                  ON ug.grimoire_id = g.grimoire_id AND ug.user_id = %s
                WHERE g.grimoire_id = %s AND {target_visibility}
                """,
                (
                    user["user_id"], payload.grimoire_id,
                    *corpus_visibility_params(user_id),
...
def delete_note(note_id: int, request: Request):
    ...
    ensure_policy_ready(user_id=user_id, surface="demo_note")
    with transaction() as (_, cur):
        visibility = demo_corpus_visibility_sql("n.grimoire_id")
        cur.execute(
            f"""
            DELETE FROM demo_note n
            WHERE n.demo_note_id = %s AND n.user_id = %s
              AND (n.grimoire_id IS NULL OR {visibility})
            RETURNING n.demo_note_id
            """,
            (note_id, user["user_id"], *corpus_visibility_params(user_id)),
        )
        if not cur.fetchone():
...
def find_similar_knowledge(
    ...
    user = _demo_user(request)
    user_id = actor_user_id(user)
    ensure_policy_ready(user_id=user_id, surface="demo_search")
    ...
    with transaction() as (_, cur):
        ...
        cur.execute(
            ...
            WHERE k.knowledge_id = %s
              ...
              AND {visibility}
            """,
            (knowledge_id, *corpus_visibility_params(user_id)),
        )
    ...
    if scores:
        with transaction() as (_, cur):
            target_visibility = demo_corpus_visibility_sql("target_section.grimoire_id")
            cur.execute(
                f"""
                SELECT e.embedding_id::text AS embedding_id,
                       e.embedding_model_id,
...
def crystallize(knowledge_id: int, request: Request):
    ...
    with transaction() as (_, cur):
        ...
        cur.execute(
            ...
            FROM knowledge k
            JOIN section s ON s.section_id = k.section_id
            WHERE k.knowledge_id = %s AND k.is_active AND {visibility}
            """,
            (knowledge_id, *corpus_visibility_params(user_id)),
        )
        if not cur.fetchone():
