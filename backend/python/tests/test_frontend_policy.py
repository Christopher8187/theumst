...
def test_public_search_policy_keeps_demo_private():
...
def test_profile_uses_approved_roles_without_exposing_demo_access_internals():
    translations = (ROOT / "frontend/shared/desktopText.js").read_text(encoding="utf-8")
    demo = (ROOT / "frontend/dashboard/src/pages/DemoPage.vue").read_text(encoding="utf-8")
    assert 'admin: "Admin"' in translations
    assert 'superadmin: "Superadmin"' in translations
...
def test_demo_visibility_users_and_email_confirmation_ui_are_present():
    ...
    app = (ROOT / "frontend/shared/DesktopApp.vue").read_text(encoding="utf-8")
    ...
    profile = (ROOT / "frontend/shared/components/ProfileContent.vue").read_text(encoding="utf-8")
