...
ALTER TABLE "user" ADD COLUMN IF NOT EXISTS authority_id int;
...
ALTER TABLE web_session ADD COLUMN IF NOT EXISTS user_id int;
ALTER TABLE web_session ADD COLUMN IF NOT EXISTS token_hash text;
ALTER TABLE web_session ADD COLUMN IF NOT EXISTS created_at timestamp NOT NULL DEFAULT now();
ALTER TABLE web_session ADD COLUMN IF NOT EXISTS expires_at timestamp;
ALTER TABLE web_session ADD COLUMN IF NOT EXISTS revoked_at timestamp;

ALTER TABLE api_key ADD COLUMN IF NOT EXISTS user_id int;
ALTER TABLE api_key ADD COLUMN IF NOT EXISTS name text;
...
def init_db(max_attempts: int = 30):
    """Create/repair the small app schema every time the backend starts.

    Docker's /docker-entrypoint-initdb.d scripts only run when a Postgres volume
    is brand new. This startup bootstrap keeps old Linux/Windows dev volumes
    from drifting into different schemas.
    """
    last_error = None
    for attempt in range(1, max_attempts + 1):
        try:
            with connect() as con, con.cursor() as cur:
                cur.execute(DB_BOOTSTRAP_SQL)
            return
        except psycopg2.OperationalError as exc:
            last_error = exc
            time.sleep(min(attempt, 5))
        except psycopg2.Error as exc:
            print("Database bootstrap error:", exc)
            raise
...
def health_assets():
    images = sorted(path.name for path in PUBLIC_IMAGES.glob("*") if path.is_file()) if PUBLIC_IMAGES.exists() else []
    return {"ok": PUBLIC_IMAGES.exists(), "image_count": len(images), "sample": images[:5]}
