...
The public site and dashboard are separate Vue builds that share the desktop, background, windows and Profile. Public windows present learning information, News and account forms. Signed-in dashboard tools provide API keys and permitted management pages. The Web Demo is a third Vue build containing the Sanctuary tree, grimoire selection, altar realms and notebook study interface. [Frontend architecture](frontend.md) describes their state and navigation.
