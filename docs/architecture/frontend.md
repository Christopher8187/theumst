...
The scene Home shortcut turns to 5 degrees; the diamond turns to 185 degrees. Each turn reaches its selected heading before enabled automatic rotation continues. Pause and dragging can interrupt the turn. Theme variables supply default button colors while component states keep priority: scene controls stay dark with light symbols, and open dock buttons keep their distinct fill in both palettes. Settings pairs its translated Language label with a language symbol.
